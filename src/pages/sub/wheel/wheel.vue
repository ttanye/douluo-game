<template>
  ...
  <view class="wheel-page">
    ...
    <view class="attrs-scroll" v-if="attributeTags.length > 0">
      <scroll-view scroll-x class="attrs-scroll-inner" enable-flex>
        <AttributeTag
...
function onResultNext() {
  ...
  if (step.id === 'godTrial' || step.id === 'godChoice') characterStore.godhood = result.label
  if (step.id === 'arc_seagod_9' && result.label.includes('海神')) characterStore.godhood = '海神'
  if (step.id === 'arc_angel_9' && result.label.includes('天使')) characterStore.godhood = '天使神'
  if (step.id === 'arc_asura_9' && result.label.includes('修罗')) characterStore.godhood = '修罗神'
  if (step.id === 'canon_godWar' && result.label.includes('海神')) characterStore.godhood = '海神'
  if (step.id === 'chapter_climax_5') {
    if (result.label.includes('海神')) characterStore.godhood = '海神'
    else if (result.label.includes('修罗')) characterStore.godhood = '修罗神'
    else if (result.label.includes('天使')) characterStore.godhood = '天使神'
  }
...
function goBack() {
  if (progressStore.phase === 'prequel' && progressStore.stepIndex <= 1) {
    uni.navigateBack()
  } else {
