<template>
  <scroll-view class="char-page" scroll-y enable-flex>
    <!-- 头像区 -->
...
const xpPercent = computed(() => Math.min(100, Math.round(characterStore.xp / characterStore.xpToNext * 100)))

const lvPower = computed(() => Math.floor(Math.pow(Math.max(1, characterStore.level), 1.4) * 1 * (innate.value / 5)))
